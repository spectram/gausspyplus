...
5. **Spatial refitting** (`spatial_fitting.py`, phases 1 and 2) — refit spectra flagged as inconsistent with neighbors, enforcing spatial coherence. An opt-in **phase 3** (`spatial_fitting/gdcluster.py`, `SpatialFitting.spatial_fitting_gdcluster()`, `refit_gdcluster` setting, default off) implements the complementary consensus-driven, per-component refit method of Liu & Du 2025 (GDCluster, arXiv:2509.16572, Sect. 2.2): every sightline (not just flagged ones) is matched against its spatial neighbors' fitted components and always refit (no AICc gate), unlike phases 1/2's flag-triggered, accept-only-if-better whole-solution swaps. Writes to a separate `_sf-p3` output file; phases 1/2 are untouched by its presence. See the module docstring for the full list of deviations from the paper.
