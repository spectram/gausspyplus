...
class GaussPyPrepare(SettingsDefault, SettingsPreparation, BaseChecks):
    ...
    def data(self):
        return (
            np.expand_dims(
                self.input_object.data[:, self.data_location[0], self.data_location[1]],
                axis=(1, 2),
            )
            if self.testing
            else self.input_object.data
        )
    ...
    def calculate_average_rms_from_data(self) -> None:
        ...
        say(
            f">> calculated rms value of {self.average_rms:.3f} from data",
            logger=self.logger,
        )
    ...
    def _save_as_pickled_file(self, results):
        ...
        suffix = (
            f"_test_Y{self.data_location[0]}X{self.data_location[1]}"
            if self.testing
            else ("" if self.suffix is None else self.suffix)
        )
    ...
    def prepare_spectrum(self, index: int) -> PreparedSpectrum:
        ...
        return PreparedSpectrum(
            intensity_values=(None if np.isnan(rms) else spectrum),
    ...
    def produce_noise_map(self, dtype="float32"):
        ...
        say(
            f"\n'{filename}' in '{os.path.dirname(path_to_file)}'",
            task="save",
            logger=self.logger,
        )
