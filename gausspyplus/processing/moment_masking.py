...
class MomentMask(object):
    ...
    def prepare_cube(self):
        ...
        if self.target_resolution_spatial is None:
            ...
            warnings.warn(
                f"No smoothing resolution specified. Will smooth to a resolution of {self.target_resolution_spatial}"
            )

        if self.target_resolution_spectral is None:
            self.target_resolution_spectral = 2 * self.current_resolution_spectral
            warnings.warn(
                f"No smoothing resolution specified. Will smooth to a resolution of {self.target_resolution_spectral}"
            )
    ...
    def moment_masking_first_steps(self):
        ...
        say(
            f"Smoothing cube spatially to a resolution of {self.target_resolution_spatial} ...",
        ...
        self.dataSmoothed, self.headerSmoothed = spatial_smoothing(
            self.data.copy(),
            self.header,
            target_resolution=self.target_resolution_spatial,
            current_resolution=self.current_resolution_spatial,
        )

        say(
            f"Smoothing cube spectrally to a resolution of {self.target_resolution_spectral} ...",
            verbose=self.verbose,
        )
        ...
        self.dataSmoothed, self.headerSmoothed = spectral_smoothing(
            self.dataSmoothed,
            self.headerSmoothed,
            target_resolution=self.target_resolution_spectral,
        ...
        if self.path_to_noise_map is None:
            ...
            say(
                f"Using rms values from {os.path.basename(self.path_to_noise_map)} for the thresholding step for the smoothed cube...",
                verbose=self.verbose,
            )
    ...
    def calculate_rms_noise(self):
        say(
            f"Determining average rms noise from {self.number_rms_spectra} spectra ...",
            verbose=self.verbose,
        )
        average_rms = calculate_average_rms_noise(
            self.dataSmoothedWithNans,
            self.number_rms_spectra,
